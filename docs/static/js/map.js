...
async function loadData() {
  ...
  try {
    ...
    try {
      ...
      // Fall back to static dataset file
    }

    if (!data || !data.features) {
      // Try local relative path for static hosting (GitHub Pages / Netlify)
      const staticPaths = [
        './data/donji_zabar_wgs84.geojson',
        'data/donji_zabar_wgs84.geojson',
        '../data/donji_zabar_wgs84.geojson'
      ];
    ...
    if (!data || !data.features) {
      throw new Error('Nije pronađen skup podataka.');
    }
    ...
    showToast('Greška pri preuzimanju parcela.');
  }
}
...
// KML Parcel Export (Server or Client-side Blob)
function downloadParcelKml(feature) {
  if (!feature) return;
  const p = feature.properties || {};
  const parcelId = String(p.parcel_id || 'parcel');

  // Try server endpoint first, fallback to client-side KML generation
  fetch(`/api/parcels/${encodeURIComponent(parcelId)}/kml`)
    .then(res => {
      if (res.ok) return res.blob();
      throw new Error('Fallback to client KML');
    })
    .then(blob => triggerBlobDownload(blob, `parcel_${parcelId.replace(/[\/\\]/g, '_')}.kml`))
    .catch(() => {
      // Client-side KML generation
      const geom = feature.geometry || {};
      const coords = geom.coordinates || [];
      let linearRing = '';
      if (geom.type === 'Polygon' && coords.length > 0) {
        linearRing = coords[0].map(pt => `${pt[0]},${pt[1]},0`).join(' ');
      } else if (geom.type === 'MultiPolygon' && coords.length > 0) {
        linearRing = coords[0][0].map(pt => `${pt[0]},${pt[1]},0`).join(' ');
      }

      const kml = `<?xml version="1.0" encoding="UTF-8"?>
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    <name>Parcela ${parcelId} - Donji Zabar</name>
    <Style id="parcelStyle">
      <LineStyle><color>ff2563eb</color><width>2.5</width></LineStyle>
      <PolyStyle><color>4d2563eb</color></PolyStyle>
    </Style>
    <Placemark>
      <name>Parcela ${parcelId}</name>
      <styleUrl>#parcelStyle</styleUrl>
      <Polygon>
        <outerBoundaryIs><LinearRing><coordinates>${linearRing}</coordinates></LinearRing></outerBoundaryIs>
      </Polygon>
    </Placemark>
  </Document>
</kml>`;
      const blob = new Blob([kml], { type: 'application/vnd.google-earth.kml+xml' });
      triggerBlobDownload(blob, `parcel_${parcelId.replace(/[\/\\]/g, '_')}.kml`);
    });
}

// Data Export Trigger (GeoJSON / CSV)
function exportData(fmt) {
  // Try server endpoint first
  fetch(`/api/export?format=${fmt}`)
    .then(res => {
      if (res.ok) return res.blob();
      throw new Error('Fallback to client export');
    })
    .then(blob => {
      const ext = fmt === 'csv' ? 'csv' : 'geojson';
      triggerBlobDownload(blob, `donji_zabar_export.${ext}`);
      showToast(`Preuzeto ${filteredFeatures.length} parcela.`);
    })
    .catch(() => {
      ...
      if (fmt === 'geojson') {
        ...
        triggerBlobDownload(blob, 'donji_zabar_export.geojson');
        showToast(`Preuzeto ${filteredFeatures.length} parcela kao GeoJSON.`);
      } else if (fmt === 'csv') {
        let csvContent = 'parcel_id,pl_broj,area_sqm,n_owners,primary_owner,primary_usage,is_coowned\n';
        filteredFeatures.forEach(f => {
          ...
          const row = [
            ...
            p.area_sqm || 0,
            p.n_owners || 0,
            `"${(p.primary_owner || '').replace(/"/g, '""')}"`,
            `"${(p.primary_usage || '').replace(/"/g, '""')}"`,
            p.is_coowned || false
          ];
          csvContent += row.join(',') + '\n';
        });
        const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
        triggerBlobDownload(blob, 'donji_zabar_export.csv');
        showToast(`Preuzeto ${filteredFeatures.length} parcela kao CSV.`);
      }
    });
}
